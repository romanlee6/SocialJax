...
- LG-TOM structure (lgtom_cnn_coop_mining.py)
"""
import sys
...
#     print("  1. Collect LLM trajectories using llms/coop_mining_llm_simulation.py")
...
def tune(default_config):
    ...
    sweep_config = {
        "name": "autoencoder_coop_mining_sweep",
        "method": "grid",  # Try all combinations
        "program": "autoencoder_cnn_coop_mining.py",  # The script to run
        "metric": {
...
@hydra.main(version_base=None, config_path="config", config_name="autoencoder_cnn_coop_mining")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
