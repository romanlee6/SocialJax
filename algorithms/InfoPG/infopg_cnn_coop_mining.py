""" 
InfoPG (Adv InfoPG with k=1) Implementation for Coop Mining Environment
...
class InfoPGPolicy(nn.Module):
    ...
    latent_size: int = 64
    action_dim: int = 8  # Coop mining has 8 actions (includes mine)
    activation: str = "relu"
...
class Transition(NamedTuple):
    ...
    prev_latents: Optional[jnp.ndarray]  # Previous latents from other agents used during forward pass
    info: jnp.ndarray
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # For coop mining, we use non-parameter sharing (PARAMETER_SHARING: False)
                ...
                for i in range(env.num_agents):
                    # Get other agents' latents that were used during forward pass
                    other_agents = [j for j in range(env.num_agents) if j != i]
                    if len(other_agents) > 0 and k_levels > 0:
                        # Get latents from other agents: [num_envs, num_others, latent_size]
                        other_latents_stored = prev_latents_state[:, other_agents, :]
                    else:
                        other_latents_stored = None
                    ...
                    transition.append(Transition(
                        ...
                        other_latents_stored,  # [num_envs, num_others, latent_size] or None
                        info_i,
                    ))
            ...
            def _update_epoch(update_state, unused, agent_idx=None):
                def _update_minbatch(train_state, batch_info, network_used):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets, network_used):
                        # RERUN NETWORK
                        # Use the stored prev_latents that were used during the forward pass
                        # This ensures consistency between forward pass and loss computation
                        pi, value, _ = network_used.apply(
                            params, 
                            traj_batch.obs, 
                            traj_batch.prev_latents,  # Use stored prev_latents from forward pass
                            k_levels,
                ...
                # Helper function to handle None values during reshaping
                def reshape_safe(x):
                    # Handle None values (prev_latents can be None when k_levels=0 or no other agents)
                    if x is None:
                        return None
                    return x.reshape((batch_size,) + x.shape[2:])
                
                batch = jax.tree_util.tree_map(
                    reshape_safe, batch, is_leaf=lambda x: x is None
                )
                
                def take_safe(x):
                    if x is None:
                        return None
                    return jnp.take(x, permutation, axis=0)
                
                shuffled_batch = jax.tree_util.tree_map(
                    take_safe, batch, is_leaf=lambda x: x is None
                )
                
                def reshape_minibatch_safe(x):
                    if x is None:
                        return None
                    return jnp.reshape(
                ...
                minibatches = jax.tree_util.tree_map(
                    reshape_minibatch_safe, shuffled_batch, is_leaf=lambda x: x is None
                )
...
def single_run(config):
    ...
    wandb.init(
        ...
        name=f'infopg_cnn_coop_mining_k{config.get("K_LEVELS", 1)}'
    )
...
@hydra.main(version_base=None, config_path="config", config_name="infopg_cnn_coop_mining")
def main(config):
    single_run(config)
